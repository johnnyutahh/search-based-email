...
better MUAs for <<my_user_profile,me>> and concurrently learn a new
search-and-tag-based email classification paradigms seems like my
biggest challenge.
...
While I may be be moving to a a search/index/tag-based
paradigm, I still need to access my 4k+ IMAP folders as I
did before, at least while I'm transitioning (presumably)
from a <<folder_based_searching,folder-based>>. Any
primary MUA/interface/IMAP-client I choose needs to
support these IMAP-folder-based paradigms (copying and
moving to folders, etc) that I currently employ with
