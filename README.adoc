...
See <<problem_details>> and https://github.com/johnnyutahh/tag-based-email/blob/master/email-tag-based-user-paradigm.txt[investigation notes] for more context.
...
2. mu can embed its metadata (tags, etc) "natively" into
   ...
   http://www.muchsync.org/[muchsync] can replicate this metadata, but
   it takes additional procuess+infrastructure.
3. #1 greatly outweights #2. As of now, notmuch "wins."
